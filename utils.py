...
def build_month_summary(info, chat_type, chat_id, year, month):
...
def build_year_summary(info, chat_type, chat_id, year):
    def format_name(user_id):
        user = {"first_name": "no_name"}
        if user_id in info.get('detach', {}):
            user = info['detach'][user_id]
...
def build_previous_month_summary(info, chat_type, chat_id, current_dt, last_run_dt=None, include_all=False):
    ...
    for offset in offsets:
        ...
        text = build_month_summary(info, chat_type, chat_id, year, month)
        if text:
...
def build_previous_year_summary(info, chat_type, chat_id, current_dt, last_run_dt=None, include_all=False):
    ...
    for year in years:
        text = build_year_summary(info, chat_type, chat_id, year)
        if text:
            summaries.append(text)
...
def find_nearest_previous_history_month(chat_type, chat_id, year, month):
    earliest_dt = get_earliest_history_dt(chat_type=chat_type, chat_id=chat_id)
    if earliest_dt is None:
        return None

    current_year = year
    current_month = month
    while (current_year, current_month) >= (earliest_dt.year, earliest_dt.month):
        file_name = datetime.datetime.strptime(f'{current_year}-{current_month}', '%Y-%m').strftime("%Y-%m")
        file_history = f"./data/{chat_type}{chat_id}/{file_name}.json"
        if os.path.exists(file_history):
...
def find_nearest_previous_history_year(chat_type, chat_id, year):
    data_dir = f"./data/{chat_type}{chat_id}/"
    if not os.path.isdir(data_dir):
        return None
    ...
    eligible_years = [y for y in years if y <= year]
